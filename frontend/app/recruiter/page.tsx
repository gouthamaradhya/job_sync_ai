      ...
      <div className="flex justify-between items-center shadow-lg bg-white dark:bg-gray-900 h-16 sticky top-0 z-50 px-4 md:px-8 border-b border-gray-100 dark:border-gray-800 transition-all duration-300">
        ...
        <div className="flex items-center gap-5">
          <NavigationMenu className="hidden md:block">
            <NavigationMenuList>
              {["Dashboard", "Jobs", "Candidates", "Help"].map((item) => (
                <NavigationMenuItem key={item}>
                  <Link href={`/recruiter/${item.toLowerCase()}`} legacyBehavior passHref>
              ...
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="rounded-full p-0 h-10 w-10 overflow-hidden">
                  <Avatar>
                    ...
                    <AvatarFallback className="bg-blue-600 text-white">JR</AvatarFallback>
                  </Avatar>
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuLabel className="font-normal">
                  <div className="flex flex-col space-y-1">
                    <p className="text-sm font-medium">Jane Recruiter</p>
